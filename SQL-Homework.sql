...
/* 2b. Find all the actors whose last names contain the letters "GEN." */
select * from actor where last_name like "%GEN%";

/* 2c. Find all actors whose last names contain the letters "LI." Order the rows by name. */
...
SELECT actor_id, last_name, first_name from actor where upper(last_name) = "WILLIAMS" AND upper(first_name) = "GROUCHO";

UPDATE actor SET actor.first_name = 'HARPO' where actor.actor_id = 172;

SELECT actor_id, last_name, first_name from actor where upper(last_name) = "WILLIAMS" AND upper(first_name) = "HARPO";

/* 4d. Perhaps we were too hasty in changing `GROUCHO` to `HARPO`. It turns out that `GROUCHO` was the correct name after all! In a single query,
if the first name of the actor is currently `HARPO`, change it to `GROUCHO`. */
SET SQL_SAFE_UPDATES=0;

UPDATE actor SET actor.first_name = 'GROUCHO' where actor.first_name = "HARPO";
...
select film.title, Count(film_actor.actor_id) as actor_count from film inner join film_actor on film.film_id = film_actor.film_id group by film_actor.film_id;

/* 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system? */
select film.title, count(inventory.film_id) from film inner join inventory on film.film_id = inventory.film_id
where film.title = "Hunchback Impossible" group by inventory.film_id;
